...
a()})})},timeQuery:function(a){a instanceof URL||(a=new URL(a));a.searchParams.set("_",Date.now());return a.href},getTimeStamp:function(){return 24===PlugSettings.settings.chatTimestamps?(new Date).toTimeString().split(" ")[0].slice(0,-3):12===PlugSettings.settings.chatTimestamps?PlugTime.getChatTimestamp():""},skip:function(){API.getDJ().id===API.getUser().id?$.post("/_/booth/skip/me"):API.moderateForceSkip()},woot:function(){API.getDJ()&&API.getDJ().id!==API.getUser().id&&$("woot").click()},meh:function(){API.getDJ()&&
API.getDJ().id!==API.getUser().id&&$("meh").click()},getWoots:function(){return API.getUsers().filter(function(a){return 1===a.vote})},getMehs:function(){return API.getUsers().filter(function(a){return-1===a.vote})},getGrabs:function(){return API.getUsers().filter(function(a){return a.grab})},searchHistory:function(a){return API.getHistory().filter(function(b){return-1<b.media.title.toLowerCase().indexOf(a.toLowerCase())})}},__chat:{log:function(a,b,c,d){this.rawLog(a,b,jplug.utils.striphtml(c),jplug.utils.striphtml(d))},
...
c.find(".rcs-small-delete").remove();var d=b.find(".text"),e=d.find(".jplug-deleted-message");d.children().length===e.length&&(b.addClass("jplug-deleted-message"),d.children().removeClass("jplug-deleted-message"))}else b.addClass("jplug-deleted-message"),(jplug.settings.mod.hideDeleted||rcs.settings.hideDeleted)&&(b.addClass("text-hidden"),rcs.Utils.hideButton(a));0<jplug.settings.mod.deletedClearup&&setTimeout(function(){0<jplug.settings.mod.deletedClearup&&(b.find("*").off(),b.empty().remove())},
jplug.settings.mod.deletedClearup)}else if(rcs.settings.improvedChat&&rcs.running&&!rcs.settings.oldChat){b.find(".contents.cid-"+a).remove();var f=b.find(".text"),e=f.find(".jplug-deleted-message");f.children().length===e.length&&(b.find("*").off(),b.empty().remove())}else b.find("*").off(),b.empty().remove()}catch(g){console.error(g,a)}}},__command:{init:function(){_$chatTriggers.chatCommand=function(a){return"/"===a.charAt(0)?(_$context.trigger("chat:command",a),!0):!1}},help:function(){}},__users:function(){var a=
[],b;for(b in jplug.other.users){if("badge"in jplug.other.users[b]){var c=jplug.utils.striphtml(jplug.other.users[b].badge);a.push("#chat .id-"+b+" .badge-box .bdg, #user-rollover.id-"+b+" .badge-box .bdg { background-image: url("+c+") !important; background-size: cover !important }");parseInt(b)===API.getUser().id&&a.push("#footer-user .badge .bdg { background-image: url("+c+") !important; background-size: cover !important }")}"color"in jplug.other.users[b]&&(c=jplug.utils.striphtml(jplug.other.users[b].color),
a.push("#chat .id-"+b+" .un, #user-lists .list .id-"+b+' .name, #waitlist .list .user[data-uid="'+b+'"] .name span { color: '+c+" !important }"))}$("head").append("<style>"+a.join("\n")+"</style>")}};jplug.utils.checkLoad();
